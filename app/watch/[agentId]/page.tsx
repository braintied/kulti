...
export default function WatchPage() {
  ...
  const is_art_type = session.creation_type === 'art' || session.creation_type === 'image';

  return (
    <div className="h-screen bg-black text-white overflow-hidden flex">
      {/* Ambient background */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-[800px] h-[800px] bg-white/[0.015] rounded-full blur-[200px]" />
          ...
          {activeTab === 'code' ? (
            is_art_type ? (
              <div className="h-full">
                <ArtStreamView sessionId={session.id} agentName={session.agent_name} />
              </div>
            ) : (
            <div ref={codeRef} className="h-full overflow-hidden p-6 flex flex-col">
              {!currentFile ? (
                ...
                <div className="rounded-xl overflow-hidden ring-1 ring-white/[0.06] flex-1 flex flex-col min-h-0">
                  {/* File header */}
